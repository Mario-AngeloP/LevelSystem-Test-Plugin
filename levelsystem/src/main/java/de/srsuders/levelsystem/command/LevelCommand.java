...
public class LevelCommand implements CommandExecutor, Messages {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(args.length == 3) {
			if(args[1].equalsIgnoreCase("add") || args[1].equalsIgnoreCase("remove")) {
				...
				try {
					...
					sender.sendMessage(prefix + "§cDie angegebene Zeichenkette: " + args[2] + " ist keine rationale Zahl.");
					return false;
				}
		...
		sender.sendMessage(prefix + "Syntax Error. Bitte Benutze: /level <Spieler> add/remove <Anzahl> um Exp hinzu zufügen oder zu entfernen.");
		return false;
	}
}
